...
<style>
    ...
    .filter-group label { display: block; font-weight: 600; margin-bottom: 4px; color: var(--text2); font-size: .82rem }
    .filter-group select, .filter-group input { width: 100%; padding: 8px 12px; border: 1.5px solid var(--border); border-radius: 8px; font-family: inherit; font-size: .88rem; background: var(--surface) }
    .filter-actions { display: flex; gap: 10px; margin-top: 16px; flex-wrap: wrap }
    .case-type-badge { display: inline-flex; align-items: center; gap: 4px; padding: 3px 10px; border-radius: 12px; font-size: .78rem; font-weight: 600 }
    ...
    .severity-high { background: #EF4444; animation: blink 1.5s infinite }
    @keyframes blink { 0%, 100% { opacity: 1 } 50% { opacity: .4 } }
    .top-emp-card { display: flex; align-items: center; gap: 12px; background: var(--surface); border-radius: var(--radius); border: 1px solid var(--border); padding: 12px 16px; transition: all .2s }
    .top-emp-card:hover { box-shadow: var(--shadow) }
    .emp-rank { width: 30px; height: 30px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: .85rem; color: #fff; flex-shrink: 0 }
    .section-title { font-size: 1.05rem; font-weight: 700; margin: 24px 0 12px; color: var(--text); display: flex; align-items: center; gap: 8px }
</style>
...
<div class="stats-grid" style="margin-bottom:20px">
    <div class="stat-card">
        ...
        <div><div class="stat-value"><?= $stats['total'] ?></div><div class="stat-label">إجمالي الحالات</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon-wrap blue"><?= svgIcon('employees', 26) ?></div>
        <div><div class="stat-value"><?= $stats['unique_employees'] ?></div><div class="stat-label">موظفون مشتبه بهم</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon-wrap red"><?= svgIcon('absent', 26) ?></div>
        <div><div class="stat-value"><?= $stats['high_severity'] ?></div><div class="stat-label">حالات خطيرة</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon-wrap purple"><?= svgIcon('lock', 26) ?></div>
        <div><div class="stat-value"><?= $stats['device_mismatch'] ?></div><div class="stat-label">أجهزة مختلفة</div></div>
    </div>
</div>
    ...
    <?php else: ?>
        <div style="overflow-x:auto">
            <table class="att-table">
                <thead>
                    <tr>
                        <th>#</th><th>التاريخ</th><th>الموظف</th><th>الفرع</th>
                        <th>النوع</th><th>الخطورة</th><th>الوصف</th><th>تفاصيل</th>
                    </tr>
                </thead>
                <tbody>
